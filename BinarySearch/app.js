...
export function binarySearch(array, value, compareFn = defaultCompare) {
  ...
  let low = 0; // 设置 low 边界，默认数组最左侧，即0
  let high = sortedArray.length - 1; // 设置 high 边界，默认数组最右侧，即array.length - 1
  while (low <= high) { // 当 low 比 high 小时
    const mid = Math.floor((low + high) / 2); // 我们计算得到中间项索引并取得中间项的值,mid
    const element = sortedArray[mid]; //存储当前mid选中的值
    // console.log('mid element is ' + element);
    if (compareFn(element, value) === Compare.LESS_THAN) { // 如果选中的值比待搜索的值小，则重新选择数组的中间值并在选中值左边的子数组中寻找（较小）
      low = mid + 1;
      // console.log('low is ' + low);
    } else if (compareFn(element, value) === Compare.BIGGER_THAN) { // 如果选中的值比待搜索的值大，则重新选择数组的中间值并在选中值右侧的子数组中寻找（较大）
      high = mid - 1;
